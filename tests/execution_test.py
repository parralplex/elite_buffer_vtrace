...
class TestExecution(unittest.TestCase):
    ...
    def test_caching_with_threads_ray(self):
        try:
            ...
            flags = change_args(op_mode="train",  lr=0.0004, batch_size=10, r_f_steps=20,
    ...
    def test_elite_insertion_and_sampling(self):
        try:
            ...
            flags = change_args(op_mode="train", lr=0.0004, batch_size=10, r_f_steps=20,
                                ...
                                multiprocessing_backend="ray",
                                environment_max_steps=500000, caching=True, caching_threads=2, cache_sample_size=2,
                                replay_parameters='[{"type": "queue", "capacity": 1, "sample_ratio": 0.5}, {"type": "custom", "capacity": 1000, "sample_ratio": 0.5, "dist_function":"ln_norm", "insert_strategy":"elite_insertion", "elite_batch_size":5, "sample_strategy":"elite_sampling", "lambda_batch_multiplier":6, "alfa_annealing_factor":2.0, "elite_sampling_strategy":"strategy4"}]')
            run_id = int(datetime.datetime.timestamp(datetime.datetime.now()))
            save_url = "results/" + env + "_" + str(run_id)
            os.makedirs(save_url)
            Learner(flags, run_id).start()
    ...
    def test_training_from_checkpoint(self):
        try:
            ...
            for i in range(2):
                ...
                flags = change_args(op_mode=op_mode, load_model_url=save_url, lr=0.0004, batch_size=10, r_f_steps=20,
                                    env=env, learner_thread_count=2, worker_count=5, envs_per_worker=3,
                                    multiprocessing_backend="ray",
