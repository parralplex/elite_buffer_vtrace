...
class ExperienceReplayProxy(ReplayBufferBase):
    def __init__(self, experience_replay, caching=True):
    ...
    def random_sample(self, batch_size):
        ...
        if self.caching:
            ...
            with self.cache_data_pos_pointer_lock:
                ...
                if wait_time > 3:
                    logger.warning(f"Learner has to wait to get batch from cache for too long = {wait_time} seconds. Try increasing cache size to speed up calculation.")
    ...
    def internal_reader(self):
        while not self.finished:
            ...
            for i in range(cache_size):
                states, actions, rewards, logits, not_done, _ = self.experience_replay.random_sample(self.batch_size)
